...
Require ($migration -match "COLUMNPROPERTY\(OBJECT_ID\('dbo\.Server_Config'\),'ID','IsIdentity'\)") 'Server_Config migration must handle identity and legacy non-identity ID schemas'
Require ($migration -match 'ROW_NUMBER\(\) OVER \(ORDER BY source\.Name\)') 'legacy Server_Config inserts must allocate deterministic IDs'
Require ($migration -notmatch 'WHEN NOT MATCHED THEN INSERT\(Name,Value\)') 'legacy Server_Config upsert must not omit mandatory ID values'
