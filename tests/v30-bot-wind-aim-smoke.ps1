...
Require ($living -match 'pkg\.WriteByte\(\(byte\)eTankCmdType\.FIRE\);\s*int wind = BaseGame\.EncodeWind\(m_game\.Wind\);\s*pkg\.WriteInt\(wind\);\s*pkg\.WriteBoolean\(wind >= 0\);\s*pkg\.WriteByte\(0\);\s*pkg\.WriteByte\(0\);\s*pkg\.WriteByte\(0\);\s*pkg\.WriteInt\(bombCount\);') 'FIRE packet must match live client wind header'
