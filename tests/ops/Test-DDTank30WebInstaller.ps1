...
foreach($token in @('DDTank30StaticPool','DDTank30Pool','103.9.156.182','8083','9300','webapps\Request','db_datareader','db_datawriter','GRANT EXECUTE','inetpub\wwwroot','Tank.Request.csproj','$requestProject')){if($raw -notmatch [regex]::Escape($token)){throw "Web installer missing token: $token"}}
