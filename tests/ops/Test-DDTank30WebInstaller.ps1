...
foreach($token in @('Get-DDTank30Instance.ps1','Apply-DDTank30Instance.ps1','DDTank30StaticPool','DDTank30Pool','webapps\Request','db_datareader','db_datawriter','GRANT EXECUTE','inetpub\wwwroot','Tank.Request.csproj','$requestProject')){if($raw -notmatch [regex]::Escape($token)){throw "Web installer missing token: $token"}}
