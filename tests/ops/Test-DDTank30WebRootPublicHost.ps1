...
try {
  ...
  [IO.File]::WriteAllText((Join-Path $stack 'webroot\gunny\login.htm'),'<a href="http://14.225.210.178/register/">Dang ky nhanh</a><a href="http://14.225.210.178:90/reg/forgotpass.html?x=1">Forgot</a><a href="https://www.facebook.com/khanhnguyennnnnn/">Forum</a>')
  [IO.File]::WriteAllText((Join-Path $stack 'webroot\index.html'),'<html><head><title>Gunny 3.0 - khanhnguyen@dev</title></head><body><p class="mySign">Hosting by <a href="https://www.facebook.com/khanhnguyennnnnn/">khanhnguyendev</a> with heart ^^</p></body></html>')
  ...
  $index=Get-Content (Join-Path $stack 'webroot\index.html') -Raw
  if(($login+$index) -match '(?i)khanhnguyennnnnn|khanhnguyen@dev|khanhnguyendev'){throw 'Third-party branding survived webroot normalization.'}
  if($index -notmatch '<title>Gunny 3\.0</title>'){throw 'Neutral Gunny 3.0 title was not preserved.'}
  if($index -match '(?i)Hosting by'){throw 'Third-party hosting footer survived webroot normalization.'}
