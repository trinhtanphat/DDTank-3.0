...
Require-Text '''game.objects.GameLocalPlayer,game.view.VaneView,ddt.manager.PathManager,game.view.Bomb,ddt.utils.RequestVairableCreater,ddt.view.character.GameCharacter''' 'Expected client classes are not selected for patching.'
Require-Text '$bombSource' 'Bomb export contract missing.'
Require-Text 'public function get target\(\) : Point' 'Bomb target getter guard missing.'
Require-Text 'public function set target\(param1:Point\) : void' 'Ruffle-safe Bomb target setter contract missing.'
Require-Text 'Ruffle-safe Bomb target setter' 'Bomb post-import verification contract missing.'
Require-Text '$requestVariableSource' 'RequestVairableCreater export contract missing.'
Require-Text 'isNaN(PlayerManager.Instance.Self.ID) ? 0 : PlayerManager.Instance.Self.ID' 'Pre-login selfid NaN guard missing.'
Require-Text 'pre-login selfid NaN guard' 'Pre-login selfid post-import verification contract missing.'
Require-Text '$gameCharacterSource' 'GameCharacter export contract missing.'
Require-Text '_ruffleActionPlayingReads' 'Ruffle projectile watchdog state missing.'
Require-Text 'SHOWGUN || this._currentAction == SHOWTHROWS || this._currentAction == SHOT || this._currentAction == THROWS' 'Ruffle projectile watchdog action scope missing.'
Require-Text 'this._currentAction.frames[0].length * 4' 'Ruffle projectile watchdog bound missing.'
Require-Text 'bounded Ruffle projectile watchdog' 'Ruffle projectile post-import verification contract missing.'
